...
Cubre: material de capa juego (partida_id None o ausente) visible siempre
dentro del workspace; material de una partida visible SOLO con esa partida
activa; nunca visible cruzando partidas, ni por listado, conteo, grafo o
acceso por ID; retrocompatibilidad total con material sin partida_id.
"""
...
def test_capa_juego_visible_sin_partida_activa():
    node = {"id": "n", "workspace": WS, "partida_id": None, "visibility": "player"}
    assert POLICY.can_view(node, _ctx()).visible


def test_material_legado_sin_clave_partida_id_visible_igual_que_hoy():
    node = {"id": "n", "workspace": WS, "visibility": "player"}  # sin la clave
    assert POLICY.can_view(node, _ctx()).visible


def test_partida_ajena_oculta_sin_partida_activa():
    node = {"id": "n", "workspace": WS, "partida_id": "partida:uno", "visibility": "player"}
...
def test_partida_otra_nunca_visible_aunque_haya_una_activa():
    node = {"id": "n", "workspace": WS, "partida_id": "partida:dos", "visibility": "player"}
...
def test_conocimiento_de_personaje_no_salta_barrera_de_partida():
    ...
    node = {
        "id": "n", "workspace": WS, "partida_id": "partida:uno",
...
def test_listado_partida_uno_ve_su_capa_mas_juego(base):
    ...
    assert ids == {"lore_dios_sol", "legacy_material_sin_partida", "partida1_pc_arden"}
    assert total == 3
...
def test_listado_partida_dos_ve_su_capa_mas_juego_nunca_la_uno(base):
    ...
    ids = {i["id"] for i in items}
    assert ids == {"lore_dios_sol", "legacy_material_sin_partida", "partida2_pc_bryn"}
...
def test_conteo_no_revela_material_de_otra_partida(base):
    ...
    assert n_uno == 3
    assert n_dos == 3  # simétrico: cada partida ve su propio material + juego
...
def test_retrocompatibilidad_material_sin_partida_igual_que_antes_de_m5a(base):
    """Un viewer clásico (sin partida activa, como antes de M5a) sigue viendo
    exactamente lo mismo que veía: la capa juego / material sin partida_id."""
    prov = _provider(base, None)
    node = prov.entity("legacy_material_sin_partida")
    assert node is not None
    node2 = prov.entity("lore_dios_sol")
