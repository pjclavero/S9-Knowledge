...
def test_cond1_lo_que_decide_es_el_SELLO_y_no_otra_cosa(tmp_path):
    ...
    * quitado el sello pero CON usuarios en la base, sigue cerrado --la segunda
      mitad del predicado, la inferencia por efecto-- y ademas el sello se
      VUELVE A ESCRIBIR, que es lo que la hace irreversible;
    * quitados el sello Y los usuarios a la vez, la pantalla VUELVE. Eso es una
      base vacia de verdad, y es lo unico que reabre.
    """
    db = tmp_path / "auth.db"
    _activar(db)
    with _con_arranque() as c:
        ...
        assert _sello(db), (
            "esa misma consulta tenia que haber REESCRITO el sello: la "
            "inferencia por efecto es pegajosa, no una cuenta viva")

        # Y ahora una base REALMENTE vacia: sin sello y sin usuarios. Se quitan
        # LAS DOS COSAS A LA VEZ a proposito; quitar solo el sello no vale,
        # porque la siguiente consulta lo vuelve a escribir.
        con = sqlite3.connect(str(db))
        con.execute("DELETE FROM install_state WHERE key = 'bootstrap_completed'")
