...
| **RK-20** | **P2** | **Tres jobs corren en cada PR pero NO son requeridos**, así que un merge puede entrar con ellos en rojo: `Especificacion JS del grafo (Node obligatorio)` (carril A), `Integridad de gates (cobertura de ramas, 0 tests, skips criticos)` y `Calibracion de gates (cada violacion tiene que ponerse ROJA)` (carril L). Corren 14 jobs; solo 11 bloquean. **Un gate que no se exige no es un gate: es un informe**, y los dos últimos existen precisamente para detectar gates que no pueden ponerse rojos | configuración de GitHub | Se arregla en los ajustes de protección de rama, no aquí. Lo que sí se ha hecho: `check_docs_consistency.py` **verifica** las cifras contra `.github/workflows/` (`ci_jobs_running` = jobs definidos; `ci_checks_required` = jobs menos los declarados no exigidos; y cada nombre de `ci_running_but_not_required` tiene que existir). Antes estos números vivían en el mismo YAML que el gate exige contrastar contra Git y nadie los miraba: ponerlos a 99/99 daba verde. **Límite declarado:** que un job sea *check requerido* vive en los ajustes de GitHub, no en el repositorio, así que el «11» se contrasta contra la aritmética declarada, no contra GitHub | **ABIERTO** — requiere acción del operador |
