...
> **HISTÓRICO (2026-07-19).** RC6 nunca llegó a crearse. Vale como registro de
> lo que se auditó entonces, no como estado. Estado vigente:
...
| URLs productivas en código de producto | Entonces: el **default de config** apuntaba a la IP productiva (RK-05). **Corregido**: hoy `viewer/app/config.py:24` usa `bolt://127.0.0.1:7687` y RK-05 está cerrado. En tests, las IPs aparecen en asserts que **verifican la redacción** de export/import |
...
- Backup real + copia al hipervisor + restore del dump en instancia aislada +
  rollback en laboratorio: **ya validados en Prioridad 1** (docs/32). No se
  repiten aquí. **Corrección 2026-08-09: aquella «copia externa» era al
  hipervisor `yggdrasil`, que ejecuta VM105 — no es off-host.**
