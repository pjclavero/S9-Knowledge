...
class PredicateSpec:
    ...
    def render(self) -> str:
        ...
        if self.confusable_with:
            partes.append(f"    no confundir con: {', '.join(self.confusable_with)}")
        return "\n".join(partes)
...
SYSTEM_PROMPT = (
    ...
    "4. Puedes proponer VARIOS predicados candidatos ordenados por tu confianza. "
    "Es preferible dar dos candidatos honestos a uno forzado.\n"
    "5. La direccion es explicita: SUBJECT_TO_OBJECT, OBJECT_TO_SUBJECT, UNDIRECTED "
    "o UNRESOLVED si el texto no lo deja claro.\n"
    ...
    "7. No decides nada: no apruebas, no fusionas entidades, no cierras vigencias. "
    "Propones. Otro sistema verifica cada cita contra el texto real y descarta lo "
    "que no aparezca.\n"
    "8. 'confidence' es tu confianza real entre 0 y 1. No copies los numeros del ejemplo."
)
...
def render_prompt(
    ...
    *,
    ...
    return (
        ...
        "2. Lista las relaciones entre esas entidades que el texto sostenga, usando "
        "solo predicados de la ontologia y citando la frase completa.\n"
        "3. Si ves una relacion que no encaja en ningun predicado, ponla en "
