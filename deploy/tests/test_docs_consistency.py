...
def sandbox(tmp_path: Path, monkeypatch) -> Sandbox:
    ...
    box.write("README.md", "# Repo\n\nSegundo.\n")
    _git(root, "add", "-A")
    _git(root, "commit", "--quiet", "-m", "segundo carril (#102)")
    ...
    _git(root, "add", "-A")
    _git(root, "commit", "--quiet", "-m", "lateral (#103)")
    ...
    box.write(
        "README.md",
        "# Repo\n\n"
        f"> **Desarrollo (`main`, commit `{head[:7]}`, último PR mergeado #102):** "
        "motor V3.\n",
    )
    box.set_status({
        "production": {
...
def test_c4c_lista_blanca_contradice_toda_rama(sandbox: Sandbox):
    ...
    assert any("limita on.push.branches" in f for f in sandbox.findings())
    assert sandbox.run() == 1
...
def test_c7_desfase_por_encima_de_la_ventana(sandbox: Sandbox):
    ...
    sandbox.write(
        "README.md",
        f"Desarrollo (`main`, commit `{sandbox.prev[:7]}`, último PR mergeado #101).\n",
    )
...
def test_c8_ultimo_pr_desfasado_con_tolerancia_cero(sandbox: Sandbox):
    sandbox.patch_dev(main_commit=sandbox.prev, latest_merged_pr=101, max_lag_commits=0)
    sandbox.write(
        "README.md",
        f"Desarrollo (`main`, commit `{sandbox.prev[:7]}`, último PR mergeado #101).\n",
    )
...
def test_c9_desfase_dentro_de_la_ventana_es_verde(sandbox: Sandbox):
    sandbox.patch_dev(main_commit=sandbox.prev, latest_merged_pr=101, max_lag_commits=3)
    sandbox.write(
        "README.md",
        f"Desarrollo (`main`, commit `{sandbox.prev[:7]}`, último PR mergeado #101).\n",
    )
...
def test_c10_commit_fuera_de_la_historia_de_main(sandbox: Sandbox):
    ...
    sandbox.write(
        "README.md",
        f"Desarrollo (`main`, commit `{sandbox.off_main[:7]}`, último PR mergeado #102).\n",
    )
